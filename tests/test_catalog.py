    ...
    def test_tree(self, catalog):
        ...
        if catalog.categories:
            ...
            bar = IncrementalBar(f'receive categories data from {catalog.name} ',
                                 max=len(list(catalog.categories)[-2:]), suffix='%(index)d/%(max)d ')
...
class TestKubotaCatalog(BaseTestCatalog):
    ...
    def test_tree(self, catalog):
        ...
        if catalog.categories:
            ...
            bar = IncrementalBar(f'receive categories data from {catalog.name} ', max=len(list(catalog.categories)[:1]), suffix='%(index)d/%(max)d ')
